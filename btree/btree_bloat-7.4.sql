...
-- WARNING: rows with is_na = 't' are known to have bad statistics (index on expression and "name" type are not supported).
-- This query is compatible with PostgreSQL 7.4 only
    ...
    FROM (
      SELECT
        ...
        max( CASE WHEN a.atttypid = 'pg_catalog.name'::regtype THEN 1 ELSE 0 END ) > 0
          OR count(1) <> i.indnatts AS is_na
      FROM pg_attribute AS a
        JOIN (
          SELECT nspname, tbl.relname AS tblname, idx.relname AS idxname, idx.reltuples, idx.relpages, idx.relam,
            indrelid, indexrelid, string_to_array(pg_catalog.textin(pg_catalog.int2vectorout(indkey)), ' ')::smallint[] AS attnum, indnatts
          FROM pg_index
